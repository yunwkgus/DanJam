...
public struct BarChartView : View {
    ...
    var frame = CGSize(width: 210, height: 140)
    ...
    public init(data:ChartData, title: String, legend: String? = nil, style: ChartStyle = Styles.barChartStyleOrangeLight, form: CGSize? = ChartForm.medium, dropShadow: Bool? = true, cornerImage:Image? = Image(systemName: "waveform.path.ecg"), valueSpecifier: String? = "%.1f",legendSpecifier: String? = "%.2f", animatedToBack: Bool = false){
        self.data = data
        self.title = title
        self.legend = legend
        self.style = style
    ...
    public var body: some View {
        ZStack{
            Rectangle()
                .foregroundColor( Color.white.opacity(0.1) ).frame(width: 280, height: 260, alignment: .center)
            ...
            VStack(alignment: .center){
                ...
                GeometryReader{ reader in
                    Rectangle()
                        ...
                        Legend(data: self.data,
                               frame: .constant(reader.frame(in: .local)), hideHorizontalLines: self.$hideHorizontalLines, specifier: legendSpecifier).frame(width: self.formSize.width + 40).offset(x: 10, y: 0)
                            .transition(.opacity)
                ...
                BarChartRow(data: data.points.map{$0.1},
                            ...
                            touchLocation: self.$touchLocation).offset(x: 50, y: 0)
