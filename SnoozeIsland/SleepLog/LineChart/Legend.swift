            ...
            ForEach((0...4), id: \.self) { height in
                HStack(alignment: .center){
                    Text("\(self.getYLegendSafe(height: height), specifier: specifier)").offset(x: 0, y: self.getYposition(height: height) )
                        .foregroundColor(Color.white) // change color
                        .font(Font.custom("AmaticSC-Regular", size:15))
                    self.line(atHeight: self.getYLegendSafe(height: height), width: self.frame.width)
                        .stroke( Color.white, style: StrokeStyle(lineWidth: 1.5, lineCap: .round, dash: [5,height == 0 ? 0 : 10])) // change color
